...
void process_file(const char* filename, bank*& pBank) {    // extracts avail for Bank, customers' alloc and max
    char buf[BUFSIZ];
    ...
    while (!finished) {
        ...
        if (first_line) {    // first line has bank's resources
            ...
            ext_vector<int> max;
            size_t size = pBank->get_avail().size();
            for (size_t i = 0; i < size; ++i) {
                alloc.add(res[i]);            // e.g., for size = 2,  0, 1
                max.add(res[i + size]);       // ditto,               2, 3
            }
            customer* c = new customer(idx++, alloc, max, pBank);
            pBank->add_customer(c);
        }
    }
    ifs.close();
}
